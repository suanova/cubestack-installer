...
if [ "\${action}" = "--delete" ]; then
    iptables -t nat -D PREROUTING -d ${FIRST_MASTER} -p tcp --dport ${REGISTRY_PORT:-5000} -j DNAT --to-destination ${FIRST_MASTER}:${REGISTRY_NODEPORT:-31148} 2>/dev/null || true
    iptables -t nat -D OUTPUT -d ${FIRST_MASTER} -p tcp --dport ${REGISTRY_PORT:-5000} -j DNAT --to-destination ${FIRST_MASTER}:${REGISTRY_NODEPORT:-31148} 2>/dev/null || true
else
    iptables -t nat -C PREROUTING -d ${FIRST_MASTER} -p tcp --dport ${REGISTRY_PORT:-5000} -j DNAT --to-destination ${FIRST_MASTER}:${REGISTRY_NODEPORT:-31148} 2>/dev/null || \
        iptables -t nat -A PREROUTING -d ${FIRST_MASTER} -p tcp --dport ${REGISTRY_PORT:-5000} -j DNAT --to-destination ${FIRST_MASTER}:${REGISTRY_NODEPORT:-31148}
    iptables -t nat -C OUTPUT -d ${FIRST_MASTER} -p tcp --dport ${REGISTRY_PORT:-5000} -j DNAT --to-destination ${FIRST_MASTER}:${REGISTRY_NODEPORT:-31148} 2>/dev/null || \
        iptables -t nat -A OUTPUT -d ${FIRST_MASTER} -p tcp --dport ${REGISTRY_PORT:-5000} -j DNAT --to-destination ${FIRST_MASTER}:${REGISTRY_NODEPORT:-31148}
fi
exit 0
FWDEOF
    chmod +x "${_FWD_SCRIPT}"
    if bash "${_FWD_SCRIPT}" --add; then
        ...
        warn "  宿主机 DNAT 应用失败(检查 iptables/root 权限)"
    fi
