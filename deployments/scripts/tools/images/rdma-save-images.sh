#!/bin/bash
# ============================================================
# rdma-save-images.sh — k8s-rdma-shared-dev-plugin 镜像: 离线下载 + 保存 tar
# 需求: 在联网/内网机器上把官方镜像下载并保存为 tar, 供离线环境(集群安装机)使用:
#       部署模块 会自动从 deployments/offline-files/rdma
#       找到这些 tar 并推送至集群内置 registry(本地源, 不联网)。
#
# ── 需要下载的镜像清单(默认 1 个; 版本可用环境变量覆盖)──
#   [rdma 插件]  ghcr.io/mellanox/k8s-rdma-shared-dev-plugin:<RDMA_IMAGE_TAG>
#
# ⚠ 镜像源: 官方已迁移到 **ghcr.io/mellanox/k8s-rdma-shared-dev-plugin**(Docker Hub 无此镜像,
#   docker pull mellanox/... 会 5 次重试失败)。必须写完整 ghcr.io 域名(短名解析依赖
...
# 文件名: <repo>_<tag>.tar(如 ghcr.io_mellanox_k8s-rdma-shared-dev-plugin_1.4.0.tar)
#
# 「保持」语义(幂等): 默认**已有 tar 则跳过**(只补缺/补新版本); 加 --force 强制重新下载覆盖。
#
# 独立运行: 本脚本**不依赖 lib-common.sh / cluster.conf / 其他脚本**, 自带最小日志/路径/skopeo
#   trust policy, 在无任何部署配置的联网准备机上可直接运行。
# 数据源: 环境变量(优先) / 内置默认(与 cluster.conf.example 声明一致)
#
# 用法:   sudo ./rdma-save-images.sh                  # 下载并保存全部镜像(已有 tar 跳过, 幂等保持)
#         sudo ./rdma-save-images.sh --list           # 只列出要下载的镜像清单(不下载)
#         sudo ./rdma-save-images.sh --force          # 强制重新下载(覆盖已有 tar)
#         sudo ./rdma-save-images.sh <镜像ref ...>    # 只下载指定镜像
# 注意: sudo 会清空环境变量, 指定版本时**必须把 VAR= 写在 sudo 之后**(否则被丢弃用默认值):
#         sudo RDMA_IMAGE_TAG=v1.5.4 ./rdma-save-images.sh
# ============================================================
set -euo pipefail
...
# ---- 版本(与 cluster.conf.example 默认一致; 环境变量可覆盖) ----
RDMA_IMAGE_TAG="${RDMA_IMAGE_TAG:-v1.5.4}"   # ⚠ 1.4.0 无 v 前缀(ghcr 上 v1.4.0 404); v1.5.1+ 带 v; 默认 v1.5.4 最新(修复 RoCE issm 硬检查)
RDMA_SAVE_DIR="${RDMA_SAVE_DIR:-${REPO_ROOT}/deployments/offline-files/rdma}"
mkdir -p "${RDMA_SAVE_DIR}"
say "配置: tag=${RDMA_IMAGE_TAG} 保存目录=${RDMA_SAVE_DIR}"
...
RDMA_IMAGE_LIST="${RDMA_IMAGE_LIST:-}"
if [ -z "${RDMA_IMAGE_LIST}" ]; then
    RDMA_IMAGE_LIST="ghcr.io/mellanox/k8s-rdma-shared-dev-plugin:${RDMA_IMAGE_TAG}"
fi
[ -n "${EXTRA_IMGS:-}" ] && RDMA_IMAGE_LIST="${EXTRA_IMGS# }"

if [ "${MODE}" = "list" ]; then
    echo "k8s-rdma-shared-dev-plugin 需下载的镜像清单(保存目录: ${RDMA_SAVE_DIR}):"
    while IFS= read -r img; do
        [ -z "${img}" ] && continue
        fname="$(echo "${img}" | sed 's#/#_#g; s#:#_#g').tar"
        if [ -f "${RDMA_SAVE_DIR}/${fname}" ]; then
            echo "  ✓ [已存在] ${img}  →  ${fname}"
        else
            echo "  ☐ [待下载] ${img}  →  ${fname}"
        fi
    done <<< "${RDMA_IMAGE_LIST}"
    exit 0
fi

save_one() {
    local src="$1" fname dest _LOCAL retry=0
    fname="$(echo "${src}" | sed 's#/#_#g; s#:#_#g').tar"
    dest="${RDMA_SAVE_DIR}/${fname}"
...
say "k8s-rdma-shared-dev-plugin 镜像清单:"
echo "${RDMA_IMAGE_LIST}" | sed 's/^/  - /'
count=0
while IFS= read -r img; do
    [ -z "${img}" ] && continue
...
echo "---------------------------------------------"
ok "保存完成: ${count} 个镜像 → ${RDMA_SAVE_DIR}"
du -sh "${RDMA_SAVE_DIR}" 2>/dev/null | awk '{print "  总大小: "$1}'
echo "  部署时由 10_rdma_shared_dev_plugin.sh 自动推送至集群内置 registry(本地源, 不联网)"
echo "  或手动: skopeo copy --src-tls-verify=false docker-archive:<tar> docker://<registry>/mellanox/k8s-rdma-shared-dev-plugin:<tag>"
