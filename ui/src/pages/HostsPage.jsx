...
function buildBootstrapScript(ip, sshUser, sshPort, t) {
  ...
  return [
    ...
    ind + remote('sudo apt-get update && sudo apt-get install -y virtinst libvirt-clients qemu-utils qemu-kvm cloud-image-utils'),
    ind,
  ...
  function submitAdd(e) {
    ...
    hostApi
      .create(
        {
          ...
          cpu_cores: form.cpu_cores ? parseInt(form.cpu_cores, 10) : null,
          memory_gb: form.memory_gb ? parseInt(form.memory_gb, 10) : null,
      ...
      .then((h) => {
        ...
        toast(t('hosts.addResult', { name: h.name, os: osLabel, lv: lvLabel }), h.os_ok && h.libvirt_ready ? 'success' : 'error')
      })
  ...
  return (
    <div>
      <div className="page-head">
        ...
        <button className="btn btn-primary" onClick={() => setShowAdd(true)}>＋ {t('hosts.add')}</button>
      </div>
      ...
      {showAdd && (
        <Modal title={t('hosts.addTitle')} onClose={() => setShowAdd(false)} width="640px">
          {/* 第一个显示项目:可复制的初始化命令(ssh-copy-id 优先) */}
          <p className="cmd-note cmd-note-top">{t('hosts.scriptHint')}</p>
          <CommandBlock script={buildBootstrapScript(form.ip.trim(), form.ssh_user, form.ssh_port, t)} />
          <p className="user-note">{t('hosts.userNote')}</p>
          <form onSubmit={submitAdd} className="add-host-form">
            <div className="grid-2">
              <Field label={t('hosts.ip')} placeholder="192.168.10.13" value={form.ip} required
                onChange={(e) => setForm({ ...form, ip: e.target.value })} />
              <Field label={t('hosts.sshUser')} value={form.ssh_user}
                onChange={(e) => setForm({ ...form, ssh_user: e.target.value })} />
            </div>
            <div className="grid-2">
              <Field label={t('hosts.sshPort')} value={form.ssh_port}
                onChange={(e) => setForm({ ...form, ssh_port: e.target.value })} />
              <Field label={t('hosts.name')} placeholder="node3" value={form.name} required
                onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div className="grid-3">
              <Field label={t('hosts.cpu')} placeholder="-" value={form.cpu_cores}
                onChange={(e) => setForm({ ...form, cpu_cores: e.target.value })} />
              <Field label={t('hosts.mem')} placeholder="-" value={form.memory_gb}
                onChange={(e) => setForm({ ...form, memory_gb: e.target.value })} />
              <Field label={t('hosts.disk')} placeholder="-" value={form.disk_gb}
                onChange={(e) => setForm({ ...form, disk_gb: e.target.value })} />
            </div>
            <p className="modal-text modal-hint">{t('hosts.addHint')}</p>
            <div className="modal-actions">
              <button type="button" className="btn btn-ghost" onClick={() => setShowAdd(false)}>{t('common.cancel')}</button>
              <button type="submit" className="btn btn-primary" disabled={busy}>
                {busy ? t('hosts.envChecking') : t('common.add')}
              </button>
            </div>
          </form>
        </Modal>
