...
function buildBootstrapScript(ip, sshUser, sshPort, t) {
  ...
  return [
    ...
    '',
    t('hosts.sc.step1'),
    // 本机(管理机)无 SSH 公钥时自动生成 ed25519 密钥
    'if [ ! -f ~/.ssh/id_ed25519.pub ] && [ ! -f ~/.ssh/id_rsa.pub ]; then',
    ind + "echo '" + t('hosts.sc.keyGen') + "'",
    ind + "ssh-keygen -t ed25519 -N '' -f ~/.ssh/id_ed25519 -q",
    'fi',
    'KEYFILE=~/.ssh/id_ed25519.pub',
    '[ -f "$KEYFILE" ] || KEYFILE=~/.ssh/id_rsa.pub',
    'ssh-copy-id -i "$KEYFILE" -p ' + p + ' ' + host,
    '',
    t('hosts.sc.step2'),
    ...
    ind + remote('command -v mc >/dev/null 2>&1 || (sudo wget -q -O /usr/local/bin/mc https://dl.minio.org.cn/client/mc/release/linux-amd64/mc && sudo chmod +x /usr/local/bin/mc)'),
    ind + remote('mc --version'),
    ind + "echo '" + t('hosts.sc.mcOk') + "'",
    'else',
