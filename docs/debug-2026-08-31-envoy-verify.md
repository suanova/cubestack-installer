...
- ⚠→✅ **修复 2(自动连通, 用户要求 "nodeport 默认走集群第一个 IP")**: nodeport 模式下节点 /etc/hosts
  的 registry.cubestack.io → REGISTRY_IP; 但 REGISTRY_IP 原自动取自 METALLB_POOL(无 MetalLB → 无服务监听)
  ...
  节点 iptables 的问题); ③ 宿主机 registry.cubestack.io:5000 由 deploy-registry.sh 自动 DNAT 到首个 master
