...
### 1.2 Chart 来源(`LWS_CHART_SOURCE`, 本地源优先)

> **默认离线安装**: 本地源(`dir`/`tgz`)优先; 仅 `oci` 源需联网。

| 方式 | 配置 | 说明 |
|---|---|---|
| **dir**(默认) | `LWS_CHART_SOURCE=dir` + `LWS_CHART_DIR=deployments/cubestack-addon/lws` | 本地解包目录(仓库已内置官方 v0.10.0 chart), 离线 |
| **tgz** | `LWS_CHART_SOURCE=tgz` + `LWS_CHART_TGZ=.../lws-chart-v0.10.0.tgz` | 本地官方发布包, 离线 |
...
### 1.3 证书管理模式(`LWS_CERT_MODE`, 二选一, 对应官方 `enableCertManager`)

| 模式 | values | 说明 | 前提 |
|---|---|---|---|
| `internal`(默认) | `enableCertManager=false` | controller 内置自签证书, 离线友好 | 无需外部组件 |
| `cert-manager` | `enableCertManager=true` | webhook 证书由 cert-manager 签发(Certificate 自动轮换) | 集群已装 cert-manager |

```bash
# 集群无 cert-manager 时用 internal(默认, 离线环境推荐)
LWS_CERT_MODE=internal sudo ./deployments/scripts/deploy-cluster.sh --enable gpu_lws
```

### 1.4 离线要求(离线优先)

> 默认按**离线安装**处理: 本地 chart 源 + 本地镜像, 全程不访问外网。

- **chart**: `deployments/cubestack-addon/lws/`(**官方 v0.10.0 完整 chart** + 发布包 tgz, 见该目录 `CUBESTACK.md`)
...
### 1.5 主要配置(cluster.conf)

| 变量 | 默认 | 说明 |
|---|---|---|
| `LWS_ENABLED` | `false` | 总开关 |
| `LWS_CHART_SOURCE` | `dir` | `dir` / `tgz` / `oci`(chart 来源) |
| `LWS_CHART_VERSION` | `v0.10.0` | 对应官方 `CHART_VERSION`(oci 源用) |
| `LWS_CHART_DIR` | `${REPO_ROOT}/deployments/cubestack-addon/lws` | 本地解包目录(内置官方 chart) |
| `LWS_CHART_TGZ` | `${LWS_CHART_DIR}/lws-chart-v0.10.0.tgz` | 本地发布包 |
| `LWS_CHART_OCI` | `oci://registry.k8s.io/lws/charts/lws` | 官方 OCI registry |
| `LWS_CERT_MODE` | `internal` | `internal`(=enableCertManager=false, 默认) / `cert-manager` |
...
| `LWS_DISAGGREGATEDSET_ENABLED` | `true` | 解耦推理支持(对应 enableDisaggregatedSet) |
...
验证内容(见 `modules/03_addon/24_verify_lws.sh`):
...
→ ⑤ 校验 `lws.io/role=leader/worker` 标签(控制器真正管理)→ ⑥ DisaggregatedSet 支持
...
Prefill 与 Decode 阶段拆分为独立 worker 组(每租户可独立副本/镜像/资源):
...
sudo kubectl delete crd leaderworkersets.leaderworkerset.x-k8s.io disaggregatedsets.disaggregatedset.x-k8s.io disaggregatedsetrolescalers.disaggregatedset.x-k8s.io
```
